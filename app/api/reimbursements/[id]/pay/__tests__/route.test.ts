...
function makeReq(url: string, session?: string): NextRequest {
  const request = new NextRequest(url, { method: "POST" });
  if (session) request.cookies.set("hermes_session", session);
  return request;
}
...
describe("POST /api/reimbursements/[id]/pay", () => {
  ...
  it("returns 401 when the user is not authenticated", async () => {
    ...
    const response = await POST(
      makeReq("http://localhost/api/reimbursements/reimb-1/pay"),
      makeContext("reimb-1"),
    );
  ...
  it("returns 404 when the reimbursement does not exist", async () => {
    verifySession.mockResolvedValue("payer-1");
    getReimbursementById.mockResolvedValue(null);

    const response = await POST(
      makeReq("http://localhost/api/reimbursements/reimb-1/pay", "valid-session"),
      makeContext("reimb-1"),
    );

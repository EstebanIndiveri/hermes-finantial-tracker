...
jest.mock("@/lib/db/schema", () => ({
  group_invitations: { token: "token", id: "invitation-id", group_id: "group-id" },
  group_members: { group_id: "group-id", user_id: "user-id" },
  users: { id: "user-id" },
  groups: {},
}));
...
describe("POST /api/join/[token]", () => {
  ...
  function mockAuthenticatedUserLookup() {
    mockDb.select = jest.fn(() => ({
      from: jest.fn(() => ({
        where: jest.fn(() => ({
          limit: jest.fn(() => Promise.resolve([{ id: "user-2" }])),
        })),
      })),
    }));
  }
